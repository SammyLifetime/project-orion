...
async function ai(redfox, event) {
    ...
    */
    ...
    if (testCommand(redfox, event, query, "image", event.senderID)) {
        ...
        */
    ...
    } else if (testCommand(redfox, event, query, "sammy", event.senderID)) {
        if (isGoingToFast(redfox, event)) return;
        let data = input.split(" ");
        if (data.length < 2) {
            ...
            try {
                ...
                const response = await openai.completions.create({
                    model: "gpt-3.5-turbo-instruct",
                    prompt: "You are Melbin an AI trained by Monsterwith, to reluctantly replies using sarcastic and funny messages.\n\nUser: " + data.join(" ") + "\nYou: ",
                    temperature: 0.5,
                    max_tokens: 2000,
                    top_p: 0.3,
                    frequency_penalty: 0.5,
        ...
        sendMessage(redfox, event, utils.formatOutput("System Info", sysinfo, "project-Sammy"));
